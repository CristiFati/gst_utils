...
class RegistryAccess:
    def __init__(self):
        if not Gst.is_initialized():
            print("Gst engine is not initialized (use: 'Gst.init()')", file=sys.stderr)
    ...
    def element_classes_dict(self, force=False):
        if not self.__element_classes_dict or force:
            ...
            # print(list(e[0] for e in items))
            for name, obj in items:
                if isinstance(obj, Gst.ElementFactory):
                    try:
                        # if "face" in name: print(name, obj)
                        self.__element_classes_dict[name] = obj.make(name).__class__
                        # if "face" in name: print("done:", self.__element_classes_dict[name])
                    except Exception:
